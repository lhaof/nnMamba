...
# from nnunet.training.network_training.nnMamba import nnMambaSeg
...
class nnUNetTrainer(NetworkTrainer):
    ...
    def setup_DA_params(self):
        ...
        print("net_num_pool_op_kernel_sizes",self.net_num_pool_op_kernel_sizes)
        if self.threeD:
            self.data_aug_params = default_3D_augmentation_params
            self.data_aug_params['rotation_x'] = (-5. / 360 * 2. * np.pi, 5. / 360 * 2. * np.pi)
            self.data_aug_params['rotation_y'] = (-5. / 360 * 2. * np.pi, 5. / 360 * 2. * np.pi)
            self.data_aug_params['rotation_z'] = (-5. / 360 * 2. * np.pi, 5. / 360 * 2. * np.pi)
            ...
            if max(self.patch_size) / min(self.patch_size) > 1.5:
                default_2D_augmentation_params['rotation_x'] = (-5. / 360 * 2. * np.pi, 5. / 360 * 2. * np.pi)
        ...
        self.data_aug_params["scale_range"] = (0.87, 1.14)
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        # print('deep_supervision_scales',self.deep_supervision_scales)
        if training:
...
#        self.network.inference_apply_nonlin = softmax_helper
        print(self.classes)
        self.network=nnMambaSeg(
            					in_ch = self.num_input_channels,
                                number_classes = len(self.classes)+1)    
#        self.network=nnMambaSegSL(
#            					in_ch = self.num_input_channels,
#                                number_classes = len(self.classes)+1)
        if torch.cuda.is_available():
            self.network.cuda()
        # if self.network.do_ds == True:


    def initialize_optimizer_and_scheduler(self):
        assert self.network is not None, "self.initialize_network must be called first"
        self.optimizer = torch.optim.Adam(self.network.parameters(), self.initial_lr, weight_decay=self.weight_decay,
                                          amsgrad=False)##default is True
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        # print(isinstance(self.network, tuple(valid)))
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():
            ...
            # self.print_to_log_file("target:",target,np.shape(target[0]))
            target = target[0]
            target = target[:, 0]
    ...
    def finish_online_evaluation(self):
        ...
        self.print_to_log_file("(interpret this as an estimate for the Dice of the different classes. This is not "
                               "exact.)")
        try:
            self.print_to_log_file("validation_result:",self.all_val_eval_metrics)
        except:
            pass
        self.online_eval_foreground_dc = []
        self.online_eval_tp = []
